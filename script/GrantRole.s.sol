...
contract GrantRole is Script, Config {
    function run() external {
        ...
        idra.grantRole(_roleHash(idra, roleName), account);
        vm.stopBroadcast();
    ...
    // CLI usage:
    // forge script script/GrantRole.s.sol:GrantRole --sig "grant(address,string,address)" <proxy> <role> <account> --private-key $KEY --broadcast
    function grant(
        address proxyAddr,
